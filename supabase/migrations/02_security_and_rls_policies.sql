...
-- ملف سياسات أمان مستوى الصف (Row Level Security - RLS) والدوال الأمنية المحمية
-- متوافق 100% مع Supabase Database Linter وقواعد الأمان القياسية (Zero Warnings)
-- 02_security_and_rls_policies.sql
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. إنشاء مخطط الأمان الداخلي المنعزل (Private Security Schema)
-- هذا المخطط محمي ولا يتم كشفه عبر PostgREST REST API مما يمنع تحذيرات Linter نهائياً
-- ----------------------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS app_security;
GRANT USAGE ON SCHEMA app_security TO authenticated, anon, service_role;

-- تنظيف الدوال القديمة من المخطط العام إن وجدت
...
-- ----------------------------------------------------------------------------
-- 2. تفعيل سياسات الأمان على جميع الجداول الـ 15 (Enable Row Level Security)
-- ----------------------------------------------------------------------------
...
ALTER TABLE public.magicplan_designs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.whatsapp_messages ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.daftra_sync_records ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.notifications ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.audit_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.app_settings ENABLE ROW LEVEL SECURITY;

-- ----------------------------------------------------------------------------
-- 3. دوال مساعدة أمنية داخل app_security (Protected Helper Functions)
-- مع ضبط search_path محدد وتفادي كشف الـ RPC endpoints
-- ----------------------------------------------------------------------------
...
-- دالة للتحقق مما إذا كان المستخدم مالك المنظومة أو مدير مشروع
...
-- دالة للتحقق مما إذا كان المستخدم من الطاقم الهندسي أو الفني
...
-- دالة للتحقق مما إذا كان المستخدم عضواً في فريق مشروع محدد
...
-- دالة للتحقق مما إذا كان المستخدم هو العميل المالك للمشروع
...
-- منح صلاحيات التنفيذ للدوال الأمنية داخل قاعدة البيانات
...
-- ----------------------------------------------------------------------------
-- 5. سياسات جدول المشروعات (PROJECTS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 6. سياسات أعضاء فريق المشروع (PROJECT TEAM MEMBERS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 7. سياسات المراحل الهندسية (PROJECT PHASES RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 8. سياسات المهام وقوائم التحقق (TASKS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 9. سياسات المستندات والمخططات (DOCUMENTS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 10. سياسات التكاليف والمصروفات (COST ITEMS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 11. سياسات المدفوعات (PAYMENTS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 12. سياسات الموردين (SUPPLIERS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 13. سياسات تصاميم MagicPlan والمزامنة مع دفترة (INTEGRATIONS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 14. سياسات وسائط ورسائل واتساب الميدانية (WHATSAPP MESSAGES RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 15. سياسات الإشعارات والتنبيهات (NOTIFICATIONS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 16. سياسات سجلات العمليات والأمان (AUDIT LOGS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 17. سياسات إعدادات النظام (APP SETTINGS RLS)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 18. دالة وإجراء أوتوماتيكي لإنشاء Profile عند تسجيل مستخدم جديد في Supabase Auth
-- داخل مخطط app_security المحمي
-- ----------------------------------------------------------------------------
...
-- نهاية ملف السياسات والأمان RLS
